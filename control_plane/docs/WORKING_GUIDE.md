...
| `entra` (= `msgraph` = `graph`) | **Microsoft Entra ID via Microsoft Graph** — client-credentials auth, follows `@odata.nextLink` pagination, flattens directory objects (users, groups, group members, service principals, applications, directory roles, role assignments). `source_options_json`: `{entity, select, filter, expand, subresource}`. | KV secret `{tenant_id, client_id, client_secret}` for an app with `Directory.Read.All` (Application) + admin consent | zero setup (needs the app consent) |
| `file` (= `dropbox`) | **ad-hoc file intake** — reads one csv/txt file or one Excel sheet (`.xlsx`/`.xls`) from OneLake as strings. Event-driven via the dropbox pipeline; each Excel tab becomes its own table. See **[`DESIGN.md`](DESIGN.md) §8**. | none (reads from `LH_Filedrop/Files`) | zero setup |
